...
#r @"bin\MCD\Debug\netcoreapp3.1\Fsharp.Data.DesignTime.dll"
...
while flag do
  ...
  if input="3" then
    ...
    if subInput="2" then
        ...
        printfn "Tweets are: %A" resp
       // let abs=JsonValue.Parse(resp)
        printfn "Want to Retweet?Y or N"
        let inputr=Console.ReadLine()
        if inputr="Y" then
           // printfn "Enter your ID:"
           // let id=Console.ReadLine()
            ...
            let str2= """","id":" """+ment +  """"}"""
            let requeststring=str1+str2;  
            let resp3=postrequest "http://localhost:8000/weatherforecast/retweet" requeststring 
            printfn "Retweeted"
